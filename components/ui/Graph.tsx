...
const ContributionGraph = ({ habit: { days, start_date, id, name, reward, icon }, handleDelete, handleReset, handleComplete }: ContributionGraphProps) => {
  ...
  return (
    <div className="card bg-base-100 shadow-xl p-4 w-full">
      {showReward && (
        <div className='text-primary text-2xl text-center p-6 '>
          <p className='text-sm md:text-md lg:text-lg  text-accent-main'> <span className='text-primary'>You</span> can now <span className='text-primary'> {reward}</span></p>
          {quote}
        </div>
      )}
      {!showReward && (
        <>
          {
            daysLeft > 0 ? (
              ...
              <div className='min-h-[120px] w-full text-center'>
                <div className="relative flex flex-col gap-4" style={{ width: '100%', minWidth: '25px' }}>
                  ...
                  <p className='text-accent text-lg'>You completed  <span className='text-primary'>{days}</span> days of  <span className='text-primary'>{name}</span></p>
                  <div className='flex justify-center'>
                    <button onClick={() => {
                      setShowReward(true)
                    ...
                    }} className='btn btn-success w-fit'>Press for Reward</button>
                  </div>
                </div>
              </div>
            )
